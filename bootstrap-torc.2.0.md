...
release: 2.0
lineage: pps-2.0
supersedes: pps-1.0 deployed 2026-08-27
assembled: 2026-09-09
adapters: Claude (agent) · Zoom Doc (human interface)
regenerate: frozen at deployment — never edited in place
integrity: 382 lines. Ends with an END OF BOOTSTRAP marker — if you cannot see it, the copy is incomplete.
---

# Bootstrap — Torc deployment

**You are an agent running in the human's work environment at Torc. This file is self-contained.**

**A deployment already exists here** — brought up 2026-08-27 from lineage `pps-1.0`. So read
*"Read this first"* below **before** anything else, and do not run Part 1.

Nothing here depends on any repository, tool, or file outside this document.

---

# Read this first — is this a fresh install or an upgrade?

**If a system built from an earlier release already exists here, do not run Part 1.**
Part 1 builds an application from nothing. Run against a live deployment it would talk over
a charter, an adapter, and a console that already work.

Ask the human, or look for it yourself: **what lineage does this environment record?**

**Look for two things: a recorded lineage, and a working system** — a console, a charter, anything this system built. Judge on the second when the first is missing.

| What you find | Do this |
|---|---|
| Lineage `pps-2.0` | Nothing. You are current. |
| An earlier lineage recorded | Apply the delta below. **Skip Part 1.** Record `pps-2.0`. |
| **No lineage, but a console or charter exists** | **This is an upgrade, not a fresh install.** Releases before 2.0 never asked deployments to record a lineage, so its absence is expected and proves nothing. Read this file's `supersedes:` frontmatter for what you were running. Apply the delta. **Skip Part 1.** Record `pps-2.0`. |
| No lineage and no system of any kind | Genuine fresh install. Run Part 1 from Step 0. |

> **If in doubt, do not run Part 1.** Ask the human. Re-running it against a live deployment
> overwrites a charter, an adapter and a console that already work; skipping it when you should
> not have costs one question.

## Delta — pps-1.0 → pps-2.0

**Nothing in Parts 2–4 changed.** The constitution and the kernel this file carries are
identical to release 1.0.

Release 2.0 is major because the *source* deployment's console needed a migration between
those releases — a section merged. That happened over there and does not reach you. Your
charter, console, and any storage you have grown are untouched, and there is nothing to
migrate.

**What 2.0 actually gives you is this section and the lineage rule below** — the machinery for
receiving a future upgrade. That is the whole payload. If you are on 1.0, applying 2.0 means:
record the new lineage, and keep working.
...
**This environment's system is now independent.** It shares a lineage with release 2.0 but evolves on its own. Re-sync deliberately at release boundaries, not continuously.

**Record the lineage somewhere you own — not in this file.** Put `lineage: pps-2.0` in the charter you wrote, or on the console. This matters more than it looks: an upgrade *replaces this file*, so a lineage recorded only here is destroyed at exactly the moment it is needed. If you cannot answer "what was I running?", the comparison that makes a safe upgrade possible is impossible.

**When an upgrade arrives**, it will be a whole new copy of this file, not a patch. Read its *"Read this first"* section, apply only the delta, and never re-run Part 1 against a live deployment.

---

<!-- END OF BOOTSTRAP — pps-2.0 — if you cannot see this line, the copy is incomplete -->
